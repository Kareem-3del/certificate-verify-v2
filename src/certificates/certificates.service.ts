...
export class CertificatesService {
  ...
  async createCertificate(
    ...
    try {
      ...
      const card = await this.createCardID({
        ...
        scale: template.id_card.scale || 0.08,
      ...
      if (template.image_cert) {
        pdf = await this.createCardID({
          ...
          scale: template.cert_card.scale,
        });
      }
  ...
  async createCardID(data: {
    ...
    const jpgImage = data.image.endsWith('.png')
        ...
        x:
          ...
          (data.positions.issued.center
            ? calcTextWidth2(
                ...
                fontSize,
                data.positions.issued.i,
              )
            : 0),
        y: data.positions.issued.y || 660,
        size:
          (data.positions.issued.fontSize || fontSize) *
            (data.positions.issued.scale || 1) || fontSize,
        color: fontColor,
    ...
    page.drawText(this.formatDate(data.expires), {
      ...
      size:
        (data.positions.expires.fontSize || fontSize) *
        (data.positions.expires.scale || 1),
      color: fontColor,
